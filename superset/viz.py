...
class NVD3TimeSeriesViz(NVD3Viz):
    ...
    def process_data(self, df, aggregate=False):
        ...
        if fill_missing:
            ...
            since = (fd.get('__from') or fd.get("since", "1 year ago"))
            from_dttm = utils.parse_human_datetime(since)
            now = datetime.now()
            if from_dttm > now:
                from_dttm = now - (from_dttm - now)
            until = fd.get('__to') or fd.get("until", "now")
            to_dttm = utils.parse_human_datetime(until)
            freq = self.get_time_domain(granularity) if self.get_time_domain(granularity) is not None else None
